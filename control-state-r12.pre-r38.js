/* Both pumping modes use the same 0–15 level scale. */
function c12Limit() { return 15; }
function c12ClampLevels() {
  const max = c12Limit();
  state.levelL = Math.max(0, Math.min(max, state.levelL));
  state.levelR = Math.max(0, Math.min(max, state.levelR));
}
function c12DosePosition() {
  if (Number.isInteger(state.dosePosition)) return Math.max(0, Math.min(2, state.dosePosition));
  const average = (state.levelL + state.levelR) / 2;
  return average <= 5 ? 0 : average <= 10 ? 1 : 2;
}
v4Track = function (side, value) {
  const max = c12Limit();
  const top = Math.round((max - value) / max * 134);
  return `<section class="v4-level c12-level c12-${max}"><label>${side.toUpperCase()} Level</label><div class="v4-track" data-v4-drag="level" data-side="${side}"><span class="ticks">${'<i></i>'.repeat(max + 1)}</span><span class="v4-level-value" style="top:${top}px">${value}</span></div></section>`;
};
v4Controls = function () {
  c12ClampLevels();
  return `<section class="v4-levels c12-levels c12-15">${v4Track('l',state.levelL)}<section class="v4-both"><label><img src="${r2Asset('control-link.svg')}" alt=""> Both</label><div class="v4-both-box" data-v4-drag="both" data-dose="${c12DosePosition()}"><span class="v4-both-lines">${'<i></i>'.repeat(7)}</span><i class="v4-both-knob"><img src="${reviewAsset('both-knob.svg')}" alt="Drag both levels"></i></div></section>${v4Track('r',state.levelR)}</section><section class="v4-speed"><h2>Speed</h2><div class="v4-speed-list">${[1,2,3,4,5].map(n => `<button data-v4="speed" data-speed="${n}" class="${state.speed===n?'active':''}">${n}</button>`).join('')}</div></section>`;
};
...
document.addEventListener('click', event => {
  ...
  if (state.auto && !state.running) { event.preventDefault(); event.stopImmediatePropagation(); return; }
  state.levelL = 5; state.levelR = 5; state.dosePosition = 0;
}, true);
document.addEventListener('click', event => {
  ...
  state.levelL = 2; state.levelR = 4; state.dosePosition = 0;
}, true);
