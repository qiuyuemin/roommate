...
AIR2_SCREEN_REGISTRY.forEach(screen => {
  ...
  const query = `build=r4&screen=${encodeURIComponent(screen.id)}`;
...
board.addEventListener('click', event => {
  ...
  if (!trigger) return;
  const frame = trigger.closest('.board-item')?.querySelector('iframe');
  frame?.contentDocument?.querySelector('[data-review="mark"]')?.click();
});
