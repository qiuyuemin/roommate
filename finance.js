...
function financeView(){
  ...
  return heading('费用 AA','','bill','记一笔')+`
    ...
    <div class="finance-summary"><div><span>总费用</span><strong>${money(sumBills(bs))}</strong><small>固定 ${money(sumBills(fixed))} · 其他 ${money(sumBills(personal))}</small></div><div><span>我的账单</span><strong>${money(myTotal(bs))}</strong></div><div><span>我的待付款</span><strong class="success">${money(due)}</strong></div></div>
    <div class="expense-columns"><section class="card"><div class="card-head"><div><h2>固定居住费用</h2></div></div>
    ...
    <section class="card personal-panel"><div class="card-head"><div><h2>其他个人费用</h2></div><button class="text-link" data-personal-add="1">＋ 记一笔</button></div>${personal.length?personal.map(b=>`<div class="personal-expense"><div class="row"><div><h3>${esc(b.title)}</h3><p>${esc(residentName(b.payer))}垫付 · ${b.members.length} 人参与</p></div><span class="amount">${money(shares(b)[state.user]||0)}<small>我的分摊 · 总额 ${money(b.amount)}</small></span></div><div class="personal-bottom">${ruleDetails(b)}${paymentButton(b)}</div></div>`).join(''):'<div class="finance-empty"><span>＋</span><h3>本月还没有其他费用</h3><button class="secondary" data-personal-add="1">记录个人费用</button></div>'}</section></div>
    <section class="card finance-ledger"><div class="card-head"><h2>我的账单明细</h2><div class="toolbar"><button data-filter="all" class="${filter==='all'?'selected':''}">全部</button><button data-filter="due" class="${filter==='due'?'selected':''}">我待结清</button></div></div>${financeTable(bs.filter(b=>filter!=='due'||debt(b,state.user)>0))}</section>`;
}
