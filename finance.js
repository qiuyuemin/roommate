const fixedTypes = {rent:'房租',electricity:'电费',water:'水费',gas:'燃气费',internet:'网费',property:'物业费',combined:'水电燃气合并账单',other:'其他固定费用'};
const commonFixedTypes=['rent','electricity','water','gas','internet','property'];
...
function classifyBill(b){
  ...
  else {const found=[['rent',/房租/],['electricity',/电费/],['water',/水费/],['gas',/燃气|煤气/],['internet',/网费|宽带/],['property',/物业费/]].find(([,re])=>re.test(b.title));b.category=found?'fixed':'personal';b.type=found?.[0]||'personal'}
}
...
function syncBillTitle(){const type=document.querySelector('#bill-type'),title=document.querySelector('#bill-title');if(type&&title)title.value=fixedTypes[type.value]||''}
...
document.addEventListener('change',e=>{
  ...
  if(e.target.id==='bill-category'){const personal=e.target.value==='personal',title=document.querySelector('#bill-title');document.querySelector('#bill-type-label').hidden=personal;if(personal){if(title&&Object.values(fixedTypes).includes(title.value))title.value=''}else syncBillTitle()}
  if(e.target.id==='bill-type')syncBillTitle();
